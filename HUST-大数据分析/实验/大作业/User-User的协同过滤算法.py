...
def recommendation(userid, movieid, user_rate, movie_user, user_user, k):
    ...
    if userid >= 100:
        print(f" {userid}\t\t{movieid}\t\t{recommend_dict[movieid]}")
    else:
        print(f" {userid}\t\t\t{movieid}\t\t{recommend_dict[movieid]}")
...
if __name__ == '__main__':
    movies, movies_title = get_movies()
    r, user_user = get_rating()
    user_rate, movie_user = get_user(r)
    user_id = int(input(f'请输入用户id: '))
    movie_id = 1
    k = int(input(f"请输入与用户id'{user_id}'最相似的前k位用户: "))
    n = int(input(f'请输入为这些用户推荐的电影数: '))
