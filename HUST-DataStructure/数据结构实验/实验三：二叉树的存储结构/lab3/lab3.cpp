...
#define LIST_INIT_SIZE 10240
#define NUM_MAX  100
typedef int Status;
typedef int TElemType;
typedef char KeyType;

static bool TNull = TRUE;
static int i = -1;
...
int main(void) {
	...
	while (op) {
		...
		case 9:
			...
			printf("输入你想新建的结点的关键字：\n");
			scanf_s("%c", &name);
			getchar();
			printf("输入它对应的数值：\n");
			scanf_s("%d", &value);
			getchar();
			printf("输入你想插入的结点位置的关键字：\n");
			scanf_s("%c", &find);
			getchar();
			printf("你想插入到该结点的左结点、右结点或根结点（左0右1根-1）：\n");
			scanf_s("%d", &LR);
			getchar();
...
Status InsertNode(TNode* T, KeyType e, int LR, TNode c) {
	...
	if (LR == -1) {
		if (newnode->data.key) c->rchild = newnode;
		else c->rchild = NULL;
		*T = c;
		return OK;
	}
...
Status ReSaveTree(TNode T) {
	if (!T) {
		fprintf(tree, "%c : %d   ", 0, 0);
		return ERROR;
	}
	fprintf(tree, "%c : %d   ", T->data.key, T->data.value);
	ReSaveTree(T->lchild);
	ReSaveTree(T->rchild);
	return OK;
}
