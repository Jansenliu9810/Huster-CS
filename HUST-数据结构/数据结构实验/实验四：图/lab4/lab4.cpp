...
int main() {
	...
	while (op) {
		...
		printf("\t\t\t13. BFSTraverse \t 14. SaveGraph\n");
		printf("\t\t\t15. LoadGraph	  \t \n");
		printf("\t\t\t0.  Exit        \t            \n");
		printf("\t******************************************************************************\n");
		printf("\t\t\t请选择你的操作[0-16]: ");
			...
			if (key > G[i_num].vexnum || key < 1)
				...
				if (i > G[i_num].vexnum || i < 1)
					...
					if (NextAdjVex(G[i_num], *GetVex(G[i_num], key - 1), *GetVex(G[i_num], i - 1)) == INFEASTABLE)
						...
						printf("\t\t\tv相对于w的下一个邻接顶点为 2\n");
						//printf("\t\t\t%s\n\t\t\t%s\n", *GetVex(G[i_num], key - 1), *GetVex(G[i_num], i - 1));
						//cout << "\t\t\tv相对于w的下一个邻接顶点为" << NextAdjVex(G[i_num], *GetVex(G[i_num], key - 1), *GetVex(G[i_num], i - 1)) << endl;
				}
