...
%token LP RP LB RB LC RC SEMI COMMA STRUCT SWITCH CASE DOT DEFAULT POUND COMMENT COMMENTBEGIN COMMENTEND VOID ASSIGNMENT  //用bison对该文件编译时，带参数-d，生成的exp.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码
%token SIZEOF SCANF PRINTF BACKLASH SINGLE_Q DOUBLE_Q BREAK CONTINUE
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN FOR DO PLUSASSIGNOP MINUSASSIGNOP AUTOPLUS AUTOMINUS
...
%token FOR_DEC ARRAY_LIST ARRAY_CALL

%left PLUSASSIGNOP MINUSASSIGNOP
...
Stmt:   Exp SEMI    {$$=mknode(EXP_STMT,$1,NULL,NULL,yylineno);}
        ...
        | FOR LP ForDec RP Stmt {$$=mknode(FOR_DEC,$3,$5,NULL,yylineno);}
        | BREAK SEMI	  {$$ = mknode(0, BREAK, NULL, NULL, yylineno);strcpy($$->type_id,"BREAK");}
        | CONTINUE SEMI	  {$$ = mknode(0, CONTINUE, NULL, NULL, yylineno);strcpy($$->type_id, "CONTINUE");}
        | DEFAULT SEMI    {$$ = mknode(0, DEFAULT, NULL, NULL, yylineno);strcpy($$->type_id,"DEFAULT");}
        ;
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
        ...
        | Exp AUTOPLUS  {$$=mknode(AUTOPLUS,$1,NULL,NULL,yylineno);strcpy($$->type_id,"AUTOPLUS");}
        | Exp PLUSASSIGNOP Exp {$$=mknode(PLUSASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"PLUSASSIGNOP");}
        | Exp AUTOMINUS {$$=mknode(AUTOMINUS,$1,NULL,NULL,yylineno);strcpy($$->type_id,"AUTOMINUS");}
        | Exp MINUSASSIGNOP Exp {$$=mknode(MINUSASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"MINUSASSIGNOP");}
